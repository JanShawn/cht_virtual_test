...
export default function Talk() {
  ...
  const speakDisplayClass =
    microState === "send" ? "border-2 border-[#EED3A4]" : "border-0";
  ...
  return (
    <div style={bgImgStyle} className="relative flex-1 overflow-hidden ">
      {/* avatar 區塊 */}
      <div className="avatar-area">
        <div className="avatar3D">放avatar</div>
      </div>
      <div className="relative mx-auto flex max-h-full max-w-[1688px] flex-col border h-screen">
        {/* setting區塊 */}
        <div className="setting-area">
          <div className={`setting-bg ${menuToggle ? "open-menu" : ""}`}></div>
          <div
            ...
            {conversation.map((data) => {
              return (
                <Fragment key={data.id}>
                  ...
                  {data.role === "avatar" && microState.includes("") && (
                    <img
                      src={loadingGif}
        ...
        {/* 提示 */}
        <div className="cursor-pointer px-4 lg:px-28">不知道要說什麼嗎？</div>
        {/* micro 區塊*/}
        <div className="z-10 flex w-full items-center justify-center p-4 lg:px-28">
          <div
            className={`mr-2 flex h-12 flex-1 items-center rounded-[40px] bg-black/80 py-2 px-4 placeholder-[#969596] outline-0 lg:mr-6 lg:px-6 ${speakDisplayClass}`}
          >
            <Scrollbars
              autoHide={true}
              ref={speakTxtElRef}
            ...
            >
              <div className="flex h-full items-center">
                ...
                {microState.includes("send") && speakTxt !== "" && (
                  <span className="speak-after whitespace-nowrap text-[#EED3A4]">
                    {speakTxt}
                  </span>
                )}
              </div>
